...
## 5. 변경 이력
### 2026-07-03
- **릴리즈 버전 업데이트 (0.1.7 -> 0.1.8)**:
  - 외부 이미지는 기본 차단을 유지하되, 설정에서 자동 표시 옵션을 켜면 버튼 없이 바로 표시되도록 지원.
  - 애플리케이션 패키지 버전, 프로젝트 메타데이터, Inno Setup 기본 버전을 `0.1.8`로 일괄 갱신.
- **릴리즈 버전 업데이트 (0.1.6 -> 0.1.7)**:
  - Outlook `.msg` 파일 열기, 본문 표시, 첨부 저장, `.msg` 파일 연결을 지원.
  - 애플리케이션 패키지 버전, 프로젝트 메타데이터, Inno Setup 기본 버전을 `0.1.7`로 일괄 갱신.
